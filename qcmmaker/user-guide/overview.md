...
| Sharing | Natively sending the portable `.qcm` file itself so the receiver obtains a local copy. Hosted HTTP links are a separate, additional option for remotely accessible quizzes. |
...
|---|---|---|
| **Create and edit a quiz** | Create a `.qcm` file, [describe and configure it](features/create-quiz/edit-information.md), then author seven [question types](features/editor/question-types/README.md). The editor supports type-specific answer options, question/answer media, correction comments, validation, reordering, fast navigation, save, and play. Match columns uses a dedicated two-column grid. | [Authoring path](features/create-quiz/README.md) |
| **Search** | Filter the home quiz list by name or keyword. | [features/search/](features/search/README.md) |
| **Workspace sorting** | Sort Home by a quiz property or use a persistent Custom order to arrange cards with long-press and drag-and-drop. | [features/workspace-sorting/](features/workspace-sorting/README.md) |
...
| **Sharing** | Natively [share the `.qcm` file itself](features/sharing/share-a-qcm-file.md) so recipients can keep and [read a local copy](features/sharing/read-a-shared-qcm-file.md). As an additional method, share a [hosted HTTP link](features/sharing/open-quiz-links.md) when the quiz is available online. | [features/sharing/](features/sharing/README.md) |
...
| **Settings** | Configure the app language, workspace access, player behavior, theme, Home shortcuts, default sorting, and quiz options menu style. | [features/settings/](features/settings/README.md) |
