---

<p align="center">
  <img src="assets/app-icon.png" alt="QcmMaker app icon" width="96" />
</p>

---

# QcmMaker — User Guide

QcmMaker is a quiz authoring, reading, practice, and sharing app for Android. It helps
you create portable `.qcm` quiz files, play them in Exam or Challenge mode, review
corrections, and send quizzes to other people or devices.

This guide explains not only where to tap, but also what each important concept means:
workspace folders, `.qcm` files, play modes, corrections, sharing, imports, and premium
access.

> For a full product overview in a single read, see [overview.md](overview.md).

## Table of Contents

- [FAQ](faq.md)

- [Getting Started](getting-started/README.md)
  - [Interface overview](getting-started/interface-overview.md)
  - [Workspace setup](getting-started/workspace-setup.md)
  - [Opening an existing `.qcm` file](getting-started/open-qcm-file/README.md)
  - [Creating your first quiz](getting-started/first-quiz.md)

- [Features](features/README.md)
  - [Creating a quiz](features/create-quiz/README.md)
  - [Editing questions](features/editor/README.md)
  - [Search](features/search/README.md)
  - [Play modes](features/play-modes/README.md)
    - [Modes de lecture — français](features/play-modes/README-fr.md)
    - [Exam question types](features/play-modes/exam-question-types/README.md)
    - [Result and replay](features/play-modes/result-and-replay.md)
  - [Import](features/import-export/README.md)
    - [Standard TXT import](features/import-export/import-txt-standard.md)
    - [Advanced TXT import](features/import-export/import-txt-advanced.md)
  - [Sharing](features/sharing/README.md)
    - [Share a quiz as a `.qcm` file](features/sharing/share-a-qcm-file.md)
    - [Read a shared `.qcm` file](features/sharing/read-a-shared-qcm-file.md)
    - [Share and open hosted quiz links](features/sharing/open-quiz-links.md)
  - [Bookmarks](features/bookmarks/README.md)
  - [Settings](features/settings/README.md)
    - [Quiz & player settings](features/settings/player/README.md)
    - [Workspace content settings](features/settings/workspace-content/README.md)
    - [Workspace behavior settings](features/settings/workspace-behaviour/README.md)
  - [Premium features](features/premium/README.md)
  - [About and contact](features/about/README.md)

---

© QmakerTech — Last updated: 2026-08-13
